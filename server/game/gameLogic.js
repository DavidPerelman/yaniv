import { createDeck, calculateHandValue, getCardValue, RANK_ORDER } from './deck.js'
...
export function applyDiscard(gameState, playerId, cards) {
  ...
  if (playerIndex !== gameState.currentPlayerIndex) return { success: false, error: 'Not your turn' }
  if (gameState.phase !== 'discard') return { success: false, error: 'Not the discard phase' }

  if (!isValidDiscard(cards)) return { success: false, error: 'Invalid discard combination' }

  const cardIds = new Set(cards.map(c => c.id))
  const player = gameState.players[playerIndex]
  const handCardIds = new Set(player.hand.map(c => c.id))
  for (const id of cardIds) {
    if (!handCardIds.has(id)) return { success: false, error: 'Card not in hand' }
  }
...
export function applyDraw(gameState, playerId, source) {
  ...
  if (playerIndex === -1) return { success: false, error: 'Player not found' }
  if (playerIndex !== gameState.currentPlayerIndex) return { success: false, error: 'Not your turn' }
  if (gameState.phase !== 'draw') return { success: false, error: 'Not the draw phase' }
  ...
  if (source === 'deck') {
    ...
    if (deck.length === 0) {
      if (discardPile.length <= 1) return { success: false, error: 'No cards left to draw' }
...
export function applyYaniv(gameState, callerId) {
  ...
  if (!canCallYaniv(caller.hand)) return { success: false, error: 'Hand value too high to call Yaniv' }
  ...
  const updatedPlayers = gameState.players.map(p => {
    if (p.isEliminated) {
      playerResults.push({ id: p.id, name: p.name, handValue: null, scoreDelta: 0, newScore: p.score, bonusApplied: null, eliminated: true })
      return p
    }
    ...
    playerResults.push({ id: p.id, name: p.name, handValue, scoreDelta, newScore, bonusApplied, eliminated })
