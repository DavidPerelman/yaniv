  ...
  const scenarios = [
    {
      ...
      action: () => emit("dev_setup_scenario", { myScore: 0, opponentScore: 0, role: "caller" }),
    },
    {
      label: "🎯 קרא יניב!",
    ...
    },
    {
      ...
      description: "אני=45 יד=5, היריב=20 יד=2 → לאחר מכן: עבור לטאב היריב ולחץ קרא יניב!",
      action: () => emit("dev_setup_scenario", { myScore: 45, opponentScore: 0, role: "receiver" }),
    },
    {
      label: "🎉 בונוס 100→50 (היריב קורא)",
      description: "אני=95 יד=5, היריב=20 יד=2 → לאחר מכן: עבור לטאב היריב ולחץ קרא יניב!",
      action: () => emit("dev_setup_scenario", { myScore: 95, opponentScore: 0, role: "receiver" }),
    },
    {
      label: "💀 פסילת יריב — אני קורא",
      description: "יריב=98, אני=0 יד=2 → יניב → יריב מקבל ~20 → נפסל",
      action: () => emit("dev_setup_scenario", { myScore: 0, opponentScore: 98, role: "caller" }),
    },
    {
      label: "🏆 ניצחון — אני קורא",
      description: "יריב=99, אני=0 יד=2 → יניב → יריב נפסל → אני מנצח",
      action: () => emit("dev_setup_scenario", { myScore: 0, opponentScore: 99, role: "caller" }),
    },
    {
      label: "😱 אסף — אני קורא",
      description: "שנינו יד=2 → אני קורא יניב → אסף! (+30)",
      action: () => {
        emit("dev_setup_scenario", { myScore: 0, opponentScore: 0, role: "caller" });
